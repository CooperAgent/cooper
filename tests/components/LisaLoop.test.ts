...
import { LISA_PHASE_COMPLETE_SIGNAL, LISA_REVIEW_APPROVE_SIGNAL, LISA_REVIEW_REJECT_PREFIX } from '../../src/renderer/types/session'

describe('Lisa Simpson Loop Types', () => {
  describe('LisaPhase', () => {
    it('should have all expected phases', () => {
      const phases: LisaPhase[] = ['plan', 'plan-review', 'execute', 'code-review', 'validate', 'final-review']
    ...
    it('should follow the correct phase order', () => {
      ...
      expect(visitedPhases).toEqual(['plan', 'plan-review', 'execute', 'code-review', 'validate', 'final-review'])
    })
  })
...
${LISA_REVIEW_REJECT_PREFIX}plan</lisa-review>`
      ...
      const rejectMatch = response.match(new RegExp(`${LISA_REVIEW_REJECT_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*(plan|execute|validate)`))
