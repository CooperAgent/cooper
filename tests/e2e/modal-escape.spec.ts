...
test.describe('Issue #108 - Modal Escape Key and Overflow', () => {
  ...
  test('03 - Open Commit modal via keyboard shortcut or button', async () => {
    ...
    const commitButton = window
      .locator('[data-testid="commit-button"]')
      .or(window.locator('button:has-text("Commit")').or(window.locator('button:has-text("Push")')))
      .first();
  ...
  test('04 - Test modal with Escape key multiple times', async () => {
    ...
    for (const trigger of modalTriggers) {
      ...
      if (isVisible) {
        ...
        const modal = window
          .locator('[role="dialog"]')
          .or(window.locator('.fixed.inset-0'))
          .first();

        const modalVisible = await modal.isVisible({ timeout: 1000 }).catch(() => false);
        if (modalVisible) {
          await window.screenshot({
            path: path.join(screenshotsDir, '06-modal-open-escape-test.png'),
          });
          ...
          await window.waitForTimeout(300);
          await window.screenshot({
            path: path.join(screenshotsDir, '07-modal-closed-escape-test.png'),
          });
          break;
        }
      }
