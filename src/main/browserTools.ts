...
export function createBrowserTools(sessionId: string): Tool<unknown>[] {
  return [
    ...
    defineTool('browser_navigate', {
      description: 'Open a web browser and navigate to a URL. The browser window will be visible to the user. Use this to access websites, web applications, or any URL. The browser persists login sessions so the user won\'t need to re-login each time.',
      parameters: z.object({
    ...
    defineTool('browser_click', {
      description: 'Click on an element in the browser. Use CSS selectors to identify the element (e.g., "button.submit", "#login-btn", "[data-testid=\'submit\']").',
      parameters: z.object({
    ...
    defineTool('browser_fill', {
      description: 'Fill a form input field with a value. Clears existing content first. Use CSS selectors to identify the input.',
      parameters: z.object({
    ...
    defineTool('browser_type', {
      description: 'Type text into an element, simulating keyboard input. Useful when you need to trigger keyboard events (like autocomplete). First clicks the element, then types.',
      parameters: z.object({
    ...
    defineTool('browser_screenshot', {
      description: 'Take a screenshot of the current browser page. Vision-capable models (GPT-5, Claude Opus/Sonnet) can analyze the image directly. For non-vision models, use browser_get_text or browser_get_html instead to read page content.',
      parameters: z.object({
        fullPage: z.boolean().optional().describe('If true, capture the full scrollable page. Default is false (viewport only).'),
        includeTextFallback: z.boolean().optional().describe('If true, also include visible text content for non-vision models. Default is false.')
      }),
      handler: async (args) => {
        ...
        if (args.includeTextFallback) {
          ...
          textFallback = 'Screenshot captured successfully. If you cannot see images, use browser_get_text to read the page content.'
        }
    ...
    defineTool('browser_get_text', {
      description: 'Get the text content from the page or a specific element. Useful for reading page content.',
      parameters: z.object({
        selector: z.string().optional().describe('CSS selector for a specific element. If omitted, returns the full page body text.')
      }),
      handler: async (args) => {
    ...
    defineTool('browser_get_html', {
      description: 'Get the HTML content from the page or a specific element. Useful for understanding page structure.',
      parameters: z.object({
        selector: z.string().optional().describe('CSS selector for a specific element. If omitted, returns the full page HTML.')
      }),
      handler: async (args) => {
    ...
    defineTool('browser_wait_for_element', {
      description: 'Wait for an element to appear on the page. Use this after actions that trigger page changes.',
      parameters: z.object({
        selector: z.string().describe('CSS selector for the element to wait for'),
        timeout: z.number().optional().describe('Maximum wait time in milliseconds. Default is 10000 (10 seconds).')
      }),
      handler: async (args) => {
    ...
    defineTool('browser_scroll', {
      ...
      parameters: z.object({
        direction: z.enum(['up', 'down', 'top', 'bottom']).optional().describe('Direction to scroll. Ignored if selector is provided.'),
    ...
    defineTool('browser_get_links', {
      description: 'Get all links on the current page. Returns up to 50 links with their text and URLs.',
      parameters: z.object({}),
      handler: async () => {
    ...
    defineTool('browser_get_form_inputs', {
      description: 'Get all form inputs on the current page. Useful for understanding what fields need to be filled.',
      parameters: z.object({}),
      handler: async () => {
    ...
    defineTool('browser_close', {
      description: 'Close the browser window for this session. The login state will be preserved for next time.',
      parameters: z.object({}),
      handler: async () => {
