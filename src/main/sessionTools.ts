...
export function createSessionTools(options: SessionToolsOptions): Tool<any>[] {
  ...
  return [
    ...
    defineTool('cooper_create_worktree_session', {
      ...
      parameters: z.object({
        ...
        baseBranch: z
          ...
          .optional()
          .describe(
            'Base branch to create the new branch from (e.g., "main", "staging"). Defaults to "main".'
          ),
        model: z
          .string()
          .optional()
      ...
      handler: async (args) => {
        ...
        const worktreeResult = await worktree.createWorktreeSession(args.repoPath, args.branch, args.baseBranch || 'main');
    ...
    defineTool('cooper_get_models', {
      description:
        'List all available AI models that can be used with Copilot. Returns model ID, name, and multiplier.',
      parameters: z.object({}),
      handler: async () => {
