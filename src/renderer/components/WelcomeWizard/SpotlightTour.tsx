...
const tourSteps: TourStep[] = [
  {
    ...
    description:
      'Each tab is a separate session with its own working directory, model, and conversation. Click + to add more!',
    position: 'right',
  },
  {
    ...
    description:
      'Paste a GitHub issue URL here to create an isolated worktree. Work on multiple issues without branch switching!',
    position: 'right',
    forceVisible: true,
  },
  {
  ...
  },
  {
    ...
    title: 'Edited Files',
    description:
      'Track files modified by Copilot. Click the commit button to push changes to your branch.',
    position: 'left',
  },
  {
    ...
    description:
      'Manage which shell commands can run automatically. Add trusted commands to skip approval prompts.',
    position: 'left',
  },
  {
...
export const SpotlightTour: React.FC<SpotlightTourProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  const [currentPosition, setCurrentPosition] = useState<'top' | 'bottom' | 'left' | 'right'>(
    'bottom'
  );
  ...
  const updateTargetPosition = useCallback(() => {
    ...
    const position =
      useExpanded && step.positionAfterClick ? step.positionAfterClick : step.position;
      ...
      switch (position) {
        ...
        case 'left': {
          ...
          const tooltipTop = Math.max(
            padding,
            Math.min(rect.top + rect.height / 2 - 60, viewportHeight - 220)
          );
          style = {
  ...
  const currentDescription =
    hasClicked && step.descriptionAfterClick ? step.descriptionAfterClick : step.description;

  // Calculate padded rect for highlight - use different padding when expanded
  const padding =
    hasClicked && step.highlightPaddingAfterClick
      ? step.highlightPaddingAfterClick
  ...
  const overlayContent = (
    <div className="fixed inset-0 z-[9999]" data-testid="spotlight-tour">
      ...
      {paddedRect && (
        <div
          ...
          style={{
            ...
            height: paddedRect.height,
            boxShadow:
              '0 0 0 4px rgba(var(--copilot-accent-rgb, 59, 130, 246), 0.3), 0 0 20px rgba(var(--copilot-accent-rgb, 59, 130, 246), 0.5)',
          }}
        />
      )}
      ...
      >
        ...
        {/* Content */}
        <h3 className="text-lg font-semibold text-copilot-text mb-2">{step.title}</h3>
        <p className="text-sm text-copilot-text-muted leading-relaxed mb-4">{currentDescription}</p>
      ...
      {!targetRect && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-copilot-bg border border-copilot-border rounded-xl shadow-2xl p-6 w-96 text-center">
          <h3 className="text-lg font-semibold text-copilot-text mb-2">{step.title}</h3>
          <p className="text-sm text-copilot-text-muted leading-relaxed mb-4">{step.description}</p>
          <p className="text-xs text-copilot-warning mb-4">(Element not visible in current view)</p>
