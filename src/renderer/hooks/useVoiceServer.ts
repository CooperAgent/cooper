...
export function useVoiceServer(options: UseVoiceServerOptions = {}) {
  ...
  const loadModel = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      if (!modelCheck.exists || !modelCheck.binaryExists) {
        ...
        console.log('[useVoiceServer] Need to download - exists:', modelCheck.exists, 'binaryExists:', modelCheck.binaryExists)
  ...
  const startRecording = useCallback(async () => {
    console.log('[useVoiceServer] startRecording called, isModelLoadedRef:', isModelLoadedRef.current)
...
async function convertToPcm(audioBlob: Blob): Promise<Uint8Array> {
  ...
  try {
    ...
    console.log('[convertToPcm] Decoded audio - duration:', audioBuffer.duration, 'channels:', audioBuffer.numberOfChannels, 'sampleRate:', audioBuffer.sampleRate)
...
function encodePcm(audioBuffer: AudioBuffer): ArrayBuffer {
  ...
  const resampledSamples = audioBuffer.sampleRate !== sampleRate
